...
class SchedulerService {
  ...
  static Future<void> performDailyAnalysis() async {
    try {
      ...
      final hasEnoughData = await NutritionAnalysisService.hasEnoughDataForAnalysis();
      if (!hasEnoughData) {
      ...
      final result = await NutritionAnalysisService.analyzeDailyNutrition(dateString);
  ...
  static Future<void> performWeeklyAnalysis() async {
    try {
      ...
      final weekStartString = '${startOfWeek.year}-${startOfWeek.month.toString().padLeft(2, '0')}-${startOfWeek.day.toString().padLeft(2, '0')}';

      // Perform the weekly analysis
      final result = await NutritionAnalysisService.generateWeeklySummary(weekStartString);
  ...
  static Future<void> _sendWeeklyAnalysisCompletedNotification() async {
    ...
    await _notifications.show(
      ...
      const NotificationDetails(
        ...
        iOS: DarwinNotificationDetails(
          categoryIdentifier: 'weekly_completed',
        ),
      ),
    );
  }
  ...
  static Future<void> debugPerformDailyAnalysis({bool force = false}) async {
    ...
    try {
      ...
      if (!force) {
        final lastAnalysisDate = await SettingsService.getLastAiAnalysisDate();
        if (lastAnalysisDate == dateString) {
          print('⚠️  DEBUG: Analysis already completed for today (use force=true to override)');
          return;
        }
      }
      ...
      if (result != null) {
        ...
        print('💡 DEBUG: Generated ${result['suggestions']?.length ?? 0} suggestions');
        print('🎯 DEBUG: Motivational quote: ${result['motivationalQuote'] ?? 'None'}');
  ...
  static Future<void> debugPerformWeeklyAnalysis({String? customWeekStart}) async {
    ...
    try {
      ...
      if (customWeekStart != null) {
        ...
        final now = DateTime.now();
        final startOfWeek = now.subtract(Duration(days: now.weekday - 1));
        weekStartString = '${startOfWeek.year}-${startOfWeek.month.toString().padLeft(2, '0')}-${startOfWeek.day.toString().padLeft(2, '0')}';
      ...
      if (result != null) {
        ...
        print('💡 DEBUG: Generated ${result['insights']?.length ?? 0} insights');
        print('🎯 DEBUG: Generated ${result['recommendations']?.length ?? 0} recommendations');
